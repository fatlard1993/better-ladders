#!/usr/bin/env python3
"""Generate Better Ladders' mod menu icon: the rope ladder, against a wall.

Planks behind it because a ladder is a thing on a wall, dimmed so the rope and
rungs carry the picture. Colours are the vanilla textures' own.
...
OUT = os.path.join(HERE, "src/main/resources/assets/better-ladders-justfatlard/icon.png")
...
def build_icon():
    wall = [[dim(px, 0.45) for px in row] for row in vanilla("block/oak_planks.png")[:16]]
    ladder = build_item()
